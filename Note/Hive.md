...
![1611899955448](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202101/29/135917-3109.png)
...
- 如果用户在查询中使用float和double类型进行比较的话，那么hive会自动把float类型转换为double类型，对于其他整数类型来说同理。也就是不同数据之间的比较其实是基于相同数据类型的比较。
...
- hive对于存储的文件的完整性一级数据内容是否和表模式相一致并没有支配能力，甚至管理表都没有给用户提供这些管理能力
