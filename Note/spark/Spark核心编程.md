...
1. a原生集合:Java/Scala中的List,但是只支持单机版! 不支持分布式!如果要做分布式的计算,需要做很多额外工作,**线程/进程通信,容错,自动均衡**.....麻烦,所以就诞生了框架!
...
coalesce()这个方法默认是不进行shuffle操作的，所以也就限制了在分区的时候只能把分区的个数**变小**，不能增大分区的个数，如果想要增大分区的个数，必须把shuffle操作设置为true,每一次调用这个方法都会生成新的Rdd，改变分区也是在新的RDD上面改变分区的个数，不会再旧的分区上面改变RDD分区的个数。
...
这种方法可以明显解决临时文件过多的问题,但是也面临其他的问题,比如排序,特别是外部排序,很耗费性能.
...
为什么把一个节点叫作Master或者是Worker，因为在Master节点上运行了一个Master daeman进程负责接收用户提交的作业，而在worker节点上面，运行着worker daeman进程，负责执行master分发的任务，所以运行master daemon进程的节点我们叫做master节点，而运行worker daemon进程的节点叫做worker节点，他们都负责自己所在的节点的事务处理。

另外，worker daemo进程还负责启动executor进程去执行作业，executor进程运行在一个容器当中。这个容器叫做Executor Backend，worker daemon就是通过Executor Backend来管理我们的Executor进程的。每一个Executor Backend只负责管理一个Executor进程,也就是说Executor BackendJVM实例持有一个Executor对象。
...
上图中我们可以看到，RDD中的每一个分区都依赖于RDDA中的三个分区，所以是shuffle操作。判断是不是宽依赖就看有没有shuffle操作，判断有没有shuffle操作就看子RDD和父RDD之间是一对一的关系还是多对一的关系，如果是多对一的关系，那么就是shuffle操作。简单来说shuffle就是一种广播的操作。
...
划分宽窄依赖主要依据是否有shuffle操作。

#### 物理执行图

##### 物理执行图的作用

![1622614085476](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/122444-666085.png)

![1622614314085](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202106/02/141158-206279.png)

![1622614451993](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/122453-827045.png)

![1622614612547](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/122458-237163.png)
...
> 到这里我们可能已经大概了解：数据的执行过程代码一般是在spark application上执行，而spark application是在Driver端，所以我们的main方法里面的代码在Driver端执行后会生成一个逻辑的执行流图，这是第一步骤，那么这个只是表示数据流向的逻辑图如何真正的放在集群中执行呢？这就需要我们的物理执行计划：
...
> 调用action算子之后，会触发Driver调用dagschedule组件，将我们的逻辑执行计划进行划分操作，也就是划分为stage和task，然后调度task在集群中执行。

![1622618380841](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202106/10/092454-532883.png)

> spark application--->job-->stage--->Task,他们之间是包含关系，并且是一对多的包含关系。

##### Job和Stage的关系

![1622618806873](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/124329-904848.png)
...
![1622619467380](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/124730-302104.png)

一个Stage中可能有多个Task，一个Stage就是一组Task在运行。

![1622619542646](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/124741-205435.png)

![1622619586695](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202106/02/153951-697348.png)

> task对应的是分区的概念。一个stage对应多个RDD（这多个RDD之间是串行依赖关系），一个RDD有多个分区，可能每一个RDD分区的个数都不同，所以我们给每一个分区划分一个task，那么一个RDD中可能有多个task，而多个RDD之间是串行的，所以多个RDD的每一个分区相互对应就可以形成一组Task集合。而一个task就像一个管道一样，在多个RDD分区之间进行流动计算。

一个Stage对应一个TaskSet，可以把TaskSet想象为线程池。

![1622619697307](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/124746-439449.png)

##### 整体执行流程

![1622619827049](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/124749-415021.png)
...
![1615078034340](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/125543-432031.png)
...
![1615084035971](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/125555-689021.png)

这样是以任务为单位进行分配map的数据，所以如果数据量很大的情况下，很可能发生内存溢出，因为存在大量的冗余数据。

**广播变量**

![1615084113558](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/125600-679784.png)
...
![1615422693537](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/125609-508967.png)
...
![1615422807548](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202111/07/125614-844581.png)
...
![20211108125549](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108125549.png)

**RDD的五大属性**

![20211108125635](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108125635.png)

- 分区列表
- 分区函数，计算如何进行分区，针对key-value数据来说。
- 依赖关系
- 计算函数，作用在每一个分片上的函数
- 最佳位置
...
![20211108130323](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108130323.png)

![20211108130424](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108130424.png)

> 说明一点：RDD很像java中的String字符串，都是不可变的，要想改变，只可以创建一个新的String，在原有的字符串上面不能改变，设计思想很类似。
> 
> 另外，RDD是分区的，但是这个分区也是逻辑上的分区，而没有真实的划分分区，而hadoop中的块，才是从物理层面划分，真实的划分为块存储。

**RDD的创建**

![20211108130932](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108130932.png)

从集合中创建：

![20211108131020](https://vscodepic.oss-cn-beijing.aliyuncs.com/pic/20211108131020.png)
