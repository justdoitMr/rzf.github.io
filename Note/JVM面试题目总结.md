...
- [JVM总结](#jvm--)
  - [描述一下JVM内存模型以及分区，需要详细介绍每隔内存区域存放什么？](#----jvm--------------------------)
  - [对象的创建方法，对象的内存分配，对象的访问定位](#-----------------------)
  - [GC的两种判定方法，以及各有什么特点](#gc----------------)
  - [GC的三种收集方法：标记清除，复制算法，标记整理算法的原理和特点，分别使用在内存的哪一个区域，如果让你优化收集方法，有什么思路？](#gc--------------------------------------------------------------)
  - [GC收集器有哪些，CMS收集器和G1收集器的特点](#gc-------cms----g1------)
  - [Minor GC,Major GC 和Full GC分别发生在什么时候，各有什么特点？](#minor-gc-major-gc--full-gc-----------------)
  - [说说常用的内存调试工具？](#------------)
  - [说说类加载的五个过程](#----------)
  - [双亲委派模型，问什么需要双亲委派模型，有什么优点？](#-------------------------)
  - [分派：静态分派和动态分派](#------------)
  - [那些内存区域会发生OOM以及会进行GC？](#---------oom-----gc-)
  - [如何打破双亲委派模型？](#-----------)
  - [新生代中区分Eden和Survivor的作用是什么](#------eden-survivor------)
  - [简述分代垃圾回收器工作流程](#-------------)
  - [GC是什么? 为什么要有 GC?](#gc-----------gc-)
  - [简述CMS收集器](#--cms---)
  - [简述G1收集器](#--g1---)
  - [简述GC中Stop the world（重点）](#--gc-stop-the-world----)
  - [垃圾回收算法（重点）](#----------)
  - [什么是分代回收算法，为什么要进行分代回收](#--------------------)
  - [你知道都有哪些垃圾回收器，各有什么特点](#-------------------)
  - [什么是内存溢出，什么是内存泄漏，有什么区别](#---------------------)
  - [对象什么时候会被GC](#--------gc)
  - [说说Java中栈内存和堆内存的区别](#--java-----------)
  - [强引用、软引用、弱引用、虚引用以及他们之间和gc的关系](#----------------------gc---)
  - [java对象创建（5种创建对象的方法）](#java-----5---------)
  - [如果对象的引用被置为 null，垃圾收集器是否会立即回收对象？](#-----------null----------------)
  - [jvm调优工具又哪些？各自的作用又是什么（重点）](#jvm---------------------)
  - [你知道哪些JVM调优参数](#-----jvm----)
  - [为什么字符串常量池在不同版本的jdk中位置会发生变化](#---------------jdk--------)
  - [概述一下类结构文件](#---------)
  - [谈谈你对jvm的理解](#----jvm---)
  - [JVM 配置常用参数有哪些?](#jvm-----------)
  - [常用 GC 调优策略有哪些？](#---gc---------)
