...
* [垃圾回收器](#垃圾回收器)
  * [GC的分类以及性能指标](#gc的分类以及性能指标)
    * [垃圾回收器的概述](#垃圾回收器的概述)
    * [java不同版本的新特性](#java不同版本的新特性)
    * [垃圾回收器的分类](#垃圾回收器的分类)
    * [评估GC的性能指标](#评估gc的性能指标)
    * [吞吐量vs暂停时间](#吞吐量vs暂停时间)
  * [不同垃圾回收器的概述](#不同垃圾回收器的概述)
    * [垃圾收集器发展史](#垃圾收集器发展史)
    * [查看默认的垃圾回收器](#查看默认的垃圾回收器)
      * [JDK8中](#jdk8中)
      * [JDK9中](#jdk9中)
    * [Serial 回收器（串行回收）](#serial-回收器串行回收)
      * [特点](#特点)
      * [Serial <strong>回收器的优势</strong>](#serial-回收器的优势)
    * [ParNew 回收器（并行回收）](#parnew-回收器并行回收)
      * [特点](#特点-1)
      * [Parnew 回收器与Serial回收器的比较](#parnew-回收器与serial回收器的比较)
    * [Parallel回收器（吞吐量优先）](#parallel回收器吞吐量优先)
      * [Parallel Scavenge <strong>回收器：吞吐量优先</strong>](#parallel-scavenge-回收器吞吐量优先)
  ...
  * [垃圾回收器的新发展](#垃圾回收器的新发展)
    ...
    * [令人震惊、革命性的 ZGC](#令人震惊革命性的-zgc)
    * [面向大堆的 AliGC](#面向大堆的-aligc)
...
![1630820818801](C:\Users\MrR\AppData\Roaming\Typora\typora-user-images\1630820818801.png)
...
![img](https://tprzfbucket.oss-cn-beijing.aliyuncs.com/hadoop/202109/05/134918-202706.jpeg)
