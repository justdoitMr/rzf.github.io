...
这两层都是属于dwd层，只不过dim层存放的是dwd层到dwm层之间的共同中间数据，在这一层需要堆数据分流操作，维度数据存放在Hbase数据库中，事实表数据存放在kafka主题当中。

**dwm**

这一层是对某一些主题进行加工，形成一个宽表。
...
source_table：从kafka数据流中读取的表数据，也就是source。

operator_type：对表的操作类型，也就是说对这张表的插入，删除，更新哪一种累哦行操作进行监控，如果三种操作都要监控，那么需要添加三条配置信息。
...
将主流中不同的表数据放到不同的位置，至于是维度表或者主题是根据sink_Type来判断。
...
这里过滤的空值是过滤掉打了delete标记的数据。注意一点，为什么可以在这里对数据进行过滤，因为我们使用Flink cdc监控数据库中的表数据，如果发生变化之后，将数据读取过来后会重新进行封装，变为json对象，里面有一个type字段类型，说明这一条数据是什么类型的，我们可以根据该字段判断这一条数据是不是脏数据。
